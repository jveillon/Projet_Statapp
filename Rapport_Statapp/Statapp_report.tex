...
\noindent
Hence, we looked for another dimensional reduction algorithm. The t-distributed Stochastic Neighbor Embedding (t-SNE) algorithm\footnote{Laurens van der Maaten and Geoffrey Hinton. Visualizing Data using t-SNE. \textit{ournal of Machine Learning Research 9} (2008)} had caught our attention, and we decided to apply it on our health data.
...
Using the estimator we obtained at the wave $i-1$, we predict GHI at $i-1$. We then add it to the socioeconomic data from the wave $i$ ($X_i$), along with the other predicted GHI from waves $j \in \llbracket 1; i-1 \rrbracket$. At the last wave, we also add the genetical data ($G$).

It can be sumed up as, for $i \in \llbracket 1; 14 \rrbracket$ :
...
To compare with the case without the genetic variables, we just do the same process while omiting the last term of the right part of the equation.
...
In this part, we only keep people still responding to the wave 14. This corresponds to $n=15487$ people.

\subsubsection{How we dealt with the missing values}

We used two lassos in this section.

The first one is the one previously introduced, the High Missing Rate Lasso. In this case, we only impute the missing values after we fitted the lasso to the data.
We also used the traditional lasso, by imputing the data at the beginning of the process.

We used the mean to impute the data as it is a usual method that does not affect directly the estimation. We only tried to impute by the median once for one of the best models and the result was close to the one with the mean, but took twice as much time to compute, so we did not use it again.

\subsubsection{Performances}

We mostly used the traditional lasso for time issues: the High Missing Rate Lasso takes approximately 3 hours to compute while the traditional one only takes 40 minutes. We also did not do an automatic grid-search with different random training and validation sets as it would have taken too much time. So, we fixed randomly at the beginning, our training, validation and testing sets. 

We tried to maximise the $R^2$ or the minimisation of the validation set's RMSE of a model. The variables we acted on to determine the best model was how we were imputating (directly at the beginning or after determining the lasso, mean or median) and the penalty $\mu$ (we tested from small values such as $\mu = 0$ or $\mu = 0.0001$ to $\mu = 20$ where only a few coefficients were left). We tried between around 20 different parameters combinations. 

With the training and validation test, we obtained that the model maximizing the $R^2$ (considering the fact there are the same model with and without the genetic data) is the traditional lasso with $\mu = 0.5$ and a imputation by the mean. The same model was also the one minimising the RMSE on the validation set. So, both indicators highlight that the mean-imputated traditional lasso with $\mu = 0.5$ is the most suitable model.
We can underline that, surprisingly, the HMLasso was less efficient than a normal lasso for all the tested values.

Concerning this model, we have for the validation set:

\begin{center}
    \begin{tabular}{|c|c|c|}
	   \hline
	   \textbf{Data} & \textbf{Validation set's $R^2$} & \textbf{Validation set's RMSE} \\
	   \hline
	   With genetic data & 0.258968 & 46.897554  \\ 
	   \hline
	   Without genetic data & 0.254326 & 47.044230 \\
	   \hline 
    \end{tabular}
\end{center}

And for the testing set - which goal is to verify there is not aberration in the sets'repartition, we get : 
...
We observe that the results from both tables are coherent with one another. 

We also observe that adding the genetic data only permits a slight $R^2$ increase, by less than $0.01$, so it is difficult, with this method, to learn something from the role genes play on someone's health.
